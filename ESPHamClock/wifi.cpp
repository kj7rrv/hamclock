...
#define BZBT_BTCOLOR    RGB565(200,200,200)     // BT plot color
...
#define DRAPPLOT_COLOR  RGB565(188,143,143)      // loading message text color
...
// time of next update attempts -- 0 will refresh immediately -- reset in initWiFiRetry()
static time_t next_sflux;
static time_t next_ssn;
static time_t next_xray;
static time_t next_kp;
static time_t next_rss;
       time_t next_sdo;                 // global for plotmgmnt.cpp::showRotatingBorder()
...
static int queryServerOverLoad (void)
{
    ...
    static int overload;
    if (!timesUp (&last_lookup_time, 100000 + randIvl(30000)))
        return (overload);
...
/* get pane rotation interval, expanded for high server load.
 */
...
static time_t nextPaneRotationTime(PlotPane pp)
{
    ...
    // find soonest rot_time that is at least interval away from all other active panes
...
#if 0
/* update ssn_spw if not recently done so by its pane.
 ...
 * return whether a new value is ready.
 */
static bool checkBzBt (time_t t)
{
    if (t < next_bzbt)
        return (false);
    ...
    if (ok) {
        ...
        // schedule next
        next_bzbt = now() + BZBT_INTERVAL;

    } else {
        // schedule retry
        next_bzbt = nextWiFiRetry();
    }
        ...
        if (BRBIsRotating()) {
            for (int i = 1; i < BRB_N; i++) {
                ...
                if (brb_rotset & (1 << next_mode)) {
                    brb_mode = next_mode;
                    Serial.printf (_FX("BR: mode rotating to %d\n"), brb_mode);
                    break;
                }
            }
        }
...
bool setPlotChoice (PlotPane pp, PlotChoice ch)
{
    ...
    if (paneIsRotating(pp))
        plot_rotationT[pp] = nextPaneRotationTime(pp);

    // ok!
    return (true);
}

/* given a plot choice and desired update interval, return and log time of next update
 */
static time_t nextPaneUpdate (PlotChoice ch, int dt)
{
    time_t next = now() + dt;
    Serial.printf (_FX("%s: next update in %d sec at %ld\n"), plot_names[ch], dt, next);
    return (next);
}
...
void updateWiFi(void)
{
    ...
    for (int i = PANE_1; i < PANE_N; i++) {
        ...
        case PLOT_CH_DEWX:
            if (t0 >= next_dewx) {
                if (updateDEWX(box))
                    next_dewx = nextPaneUpdate (ch, DEWX_INTERVAL);
                else
        ...
        case PLOT_CH_DXWX:
            if (t0 >= next_dxwx) {
                if (updateDXWX(box))
                    next_dxwx = nextPaneUpdate (ch, DXWX_INTERVAL);
                else
        ...
        case PLOT_CH_FLUX:
            if (t0 >= next_sflux) {
                if (updateSolarFlux(box))
                    next_sflux = nextPaneUpdate (ch, SFLUX_INTERVAL);
                else
                    next_sflux = nextWiFiRetry();
            }
            break;
        ...
        case PLOT_CH_KP:
            if (t0 >= next_kp) {
                if (updateKp(box))
                    next_kp = nextPaneUpdate (ch, KP_INTERVAL);
                else
                    next_kp = nextWiFiRetry();
            }
            break;
        ...
        case PLOT_CH_MOON:
            if (t0 >= next_moon) {
                ...
                next_moon = nextPaneUpdate (ch, MOON_INTERVAL);
            }
            break;
        ...
        case PLOT_CH_NOAASWX:
            if (t0 >= next_noaaswx) {
                if (updateNOAASWx(box))
                    next_noaaswx = nextPaneUpdate (ch, NOAASWX_INTERVAL);
                else
        ...
        case PLOT_CH_SSN:
            if (t0 >= next_ssn) {
                if (updateSunSpots(box))
                    next_ssn = nextPaneUpdate (ch, SSPOT_INTERVAL);
                else
                    next_ssn = nextWiFiRetry();
            }
            break;
        ...
        case PLOT_CH_XRAY:
            if (t0 >= next_xray) {
                if (updateXRay(box))
                    next_xray = nextPaneUpdate (ch, XRAY_INTERVAL);
                else
                    next_xray = nextWiFiRetry();
            }
            break;
        ...
        case PLOT_CH_SDO:
            if (t0 >= next_sdo) {
                if (updateSDO(box))
                    next_sdo = nextPaneUpdate (ch, (isSDORotating() ? SDO_ROT_INTERVAL : SDO_INTERVAL));
                else
        ...
        case PLOT_CH_SOLWIND:
            if (t0 >= next_swind) {
                if (updateSolarWind(box))
                    next_swind = nextPaneUpdate (ch, SWIND_INTERVAL);
                else
        ...
        case PLOT_CH_DRAP:
            if (t0 >= next_drap) {
                if (updateDRAPPlot(box))
                    next_drap = nextPaneUpdate (ch, DRAPPLOT_INTERVAL);
                else
        ...
        case PLOT_CH_CONTESTS:
            if (t0 >= next_contests) {
                if (updateContests(box))
                    next_contests = nextPaneUpdate (ch, CONTESTS_INTERVAL);
                else
        ...
        case PLOT_CH_PSK:
            if (t0 >= next_psk) { 
                if (updatePSKReporter(box))
                    next_psk = nextPaneUpdate (ch, PSK_INTERVAL);
                else
        ...
        case PLOT_CH_OTA:
            if (t0 >= next_ota) { 
                if (updateOnTheAir(box))
                    next_ota = nextPaneUpdate (ch, OTA_INTERVAL);
                else
        ...
        case PLOT_CH_BZBT:
            if (t0 >= next_bzbt) { 
                if (updateBzBt(box))
                    next_bzbt = nextPaneUpdate (ch, BZBT_INTERVAL);
                else
                    next_bzbt = nextWiFiRetry();
            }
            break;
    ...
    if (t0 >= next_wwx) {
        if (prop_map == PROP_MAP_OFF && core_map == CM_WX)
...
void sendUserAgent (WiFiClient &client)
{
    ...
    if (logUsageOk()) {
        ...
        // label spots: 0 no, 1 prefix, 2 call
...
static bool updateBzBt (const SBox &box)
{
    ...
    if (ok) {
        ...
        plotMessage (box, BZBT_BZCOLOR, _FX("BzBt data short or old"));
    }
...
{
    return (checkSolarFlux(t0) || checkKp(t0) || checkXRay(t0) || checkBzBt(t0));
}
