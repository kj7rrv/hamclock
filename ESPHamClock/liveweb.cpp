...
static void updateExistingClient (ws_cli_conn_t *client)
{
    ...
    // always send header
    unsigned n_hdrsent = ws_sendframe_bin (client, (const char *) hdr, sizeof(hdr));
    if (n_hdrsent != sizeof(hdr))
        printf ("LIVE: client %s: wrong header write %u != %d\n",
                                ws_getaddress(client), n_hdrsent, (int)sizeof(hdr));
    ...
    if (live_verbose > 1) {
        ...
        printf ("LIVE: client %s: write hdr %d bytes and update took %ld usec\n",
                        ws_getaddress(client), (int)sizeof(hdr), TVDELUS (tv0,tv1));
    }
...
static void ws_onmessage (ws_cli_conn_t *client, const unsigned char *msg, uint64_t size, int type)
{
    ...
    // check
    if (!found)
        printf ("LIVE: unknown ws command: %s\n", cmd);
}
